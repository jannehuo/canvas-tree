...
exports.randomPointFromCircle = randomPointFromCircle;
exports.getRelativeTo = getRelativeTo;
exports.getDistance = getDistance;
exports.getDirection = getDirection;

function randomPointFromCircle(radius) {
  var angle = Math.random() * 2 * Math.PI;
  var radius_sq = Math.random() * radius * radius;
  var x = Math.sqrt(radius_sq) * Math.cos(angle);
  var y = Math.sqrt(radius_sq) * Math.sin(angle);
  return {
    x: x,
    y: y
  };
...
function getRelativeTo(v1, v2) {
  var relativeToTarget = {
    x: v1.x - v2.x,
    y: v1.y - v2.y
  };
...
function getDistance(relative) {
  var distance = Math.sqrt(relative.x * relative.x + relative.y * relative.y);
  return distance;
}

function getDirection(relativeToTarget, distance) {
  if (distance === 0) {
    return {
      x: 0,
      y: 0
...
function () {
  ...
  _createClass(Leaf, [{
    ...
    key: "getForce",
    value: function getForce(distance) {
      var force = distance * (1 / 10);
      return force;
    }
  }, {
    key: "getRepel",
